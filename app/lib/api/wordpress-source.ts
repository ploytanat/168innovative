...
const HERO_THEMES: HeroSlideView["theme"][] = [
  "rose",
  "sky",
  "emerald",
  "violet",
...
function chooseSyntheticVariantGroup(products: ProductView[], locale: Locale) {
  const preferred = [
    "model",
    "sku",
    "variant type",
      ...
      label:
        key === "model"
          ? locale === "th"
            ? "รุ่น"
            : "Model"
          : (products[0]?.specs.find(
              (spec) => normalizeToken(spec.label) === key,
            )?.label ?? (locale === "th" ? "ตัวเลือก" : "Option")),
      values: new Map(
        products.map((product, index) => [product.id, values[index]!]),
      ),
    };
  }
...
function buildGroupedProductView(
  ...
  const variants = members.map((member) => {
    const selectedVariant =
      member.variants.find(
        (variant) => variant.slug === member.defaultVariantSlug,
      ) ?? member.variants[0];
    ...
    return {
      ...
      gallery: selectedVariant?.gallery?.length
        ...
        : member.gallery,
      specs: selectedVariant?.specs?.length
      ...
      options: [
        {
          ...
          valueLabel:
            variantGroup.values.get(member.id) ??
            member.sku ??
            member.name ??
            member.slug,
        },
  ...
  const groupedProduct: ProductView = {
    ...
    gallery: defaultVariant?.gallery?.length
...
function resolveMediaImage(
  ...
  return {
    src,
    alt:
      trimText(media?.alt_text) ||
      trimText(media?.title?.rendered) ||
      fallbackAlt ||
      "",
  };
}
...
function buildImageView(options: {
  alt?: string;
  fallbackSrc?: string;
  image?:
    | number
  ...
  if (value && typeof value === "object") {
    const normalized = Object.entries(value as Record<string, unknown>)
      .map(([key, nestedValue]) => {
        ...
        return formatted
          ? `${humanizeSpecLabel(key)}: ${formatted}`
          : undefined;
      })
      .filter((item): item is string => Boolean(item));
...
function mapSpecs(specsInput: unknown): ProductSpecView[] {
  ...
  if (Array.isArray(value)) {
    return value.flatMap((item) => {
      ...
      const label =
        trimText(String(row.spec_label ?? row.label ?? row.key ?? "")) ||
        undefined;
      const formattedValue = formatSpecValue(row.spec_value ?? row.value);

      return label && formattedValue ? [{ label, value: formattedValue }] : [];
    });
  }
...
function ensureGallery(primaryImage?: ImageView, gallery: ImageView[] = []) {
  const entries = [primaryImage, ...gallery].filter((item): item is ImageView =>
    Boolean(item?.src),
  );
...
function getEmbeddedFeaturedMedia(entry: {
  _embedded?: {
...
function mapVariantOptions(
  ...
  return (options ?? []).flatMap((option) => {
    ...
    const valueKey =
      ...
      trimText(pickLocalizedText(locale, option?.value_th, option?.value_en));
...
async function fetchWordPressCompanyRaw() {
  const items = await fetchWordPressCollection<WPCompanyEntry>(
    "wp/v2/company",
    {
...
async function fetchWordPressHomePageRaw() {
  const items = await fetchWordPressCollection<WPSinglePageEntry>(
    "wp/v2/pages",
    {
      searchParams: {
        slug: "home",
...
function mapCategoryTermToView(
  term: WPCategoryTerm,
  locale: Locale,
): CategoryView {
  ...
  return {
    ...
    introHtml: normalizeRichText(
      pickLocalizedText(
        locale,
        term.acf?.intro_html_th,
        term.acf?.intro_html_en,
      ),
    ),
    faqItems: mapFaqItems(term.acf?.faq_items, locale),
    seoTitle: pickLocalizedText(
      locale,
      term.acf?.seo_title_th,
      term.acf?.seo_title_en,
      name,
    ),
  ...
  return blocks.reduce<ArticleBlockView[]>((accumulator, block) => {
    ...
    if (type === "cta") {
      ...
      const buttonLabel = trimText(
        pickLocalizedText(locale, block.button_label_th, block.button_label_en),
      );
      ...
      accumulator.push({
        ...
        style:
          ...
          block.style === "note"
            ...
            : block.style,
        heading,
        body: trimText(pickLocalizedText(locale, block.body_th, block.body_en)),
        buttonLabel,
...
async function loadWordPressProductViews(locale: Locale) {
  ...
  const mappedProducts = rawProducts.map((product) => {
    const embeddedCategory = getEmbeddedTerms(
      product,
      "product_category",
    )[0] as (WPTerm & WPCategoryTerm) | undefined;
    ...
    const primaryImage = buildImageView({
      alt: fallbackAlt,
      ...
      product.acf?.variants?.length
        ? product.acf.variants.map((variant, index) => {
            ...
            return {
              ...
              slug:
                trimText(variant.slug) ||
                `${product.slug}-variant-${index + 1}`,
    ...
    const defaultVariant =
      variants.find(
        (variant) => variant.slug === product.acf?.default_variant_slug,
      ) ?? variants[0];
    const specs = defaultVariant?.specs.length
      ? defaultVariant.specs
      : mapSpecs(product.acf?.specs_json);
    ...
    return {
      product: {
        ...productView,
        variantSummary:
          productView.variantGroups.map((group) => group.label).join(" / ") ||
          undefined,
        searchText: buildProductSearchText(productView),
      },
      relatedProductIds: extractRelatedProductIds(
        product.acf?.related_products,
      ),
    };
  });
...
export async function getWordPressCategories(locale: Locale) {
  ...
  return rawCategories.map((category) =>
    mapCategoryTermToView(category, locale),
  );
}
...
export async function getWordPressAllProductsForSitemap(): Promise<
  ProductSitemapView[]
> {
  const rawProducts = await fetchWordPressProductsRaw();
...
> {
  ...
  const productsEn = new Map(
    (await loadWordPressProductViews("en")).map((product) => [
      product.slug,
      product,
    ]),
  );
  const rawProducts = new Map(
    (await fetchWordPressProductsRaw()).map((product) => [
      product.slug,
      product,
    ]),
  );
...
export async function getWordPressArticles(
  ...
  return rawArticles.map((article) => {
    ...
    return {
      ...
      updatedAt:
        trimText(article.acf?.updated_at) || article.modified || article.date,
      seoTitle: trimText(
        pickLocalizedText(
          locale,
          article.acf?.seo_title_th,
          article.acf?.seo_title_en,
        ),
      ),
...
export async function getWordPressArticleBySlug(
  ...
  return (
    (await getWordPressArticles(locale)).find(
...
export async function getWordPressAbout(
  ...
  return {
    ...
    seoTitle: trimText(
      pickLocalizedText(
        locale,
        entry.acf?.seo_title_th,
        entry.acf?.seo_title_en,
        heroTitle,
      ),
    ),
...
export async function getWordPressCompany(
  locale: Locale,
...
export async function getWordPressHeroSlides(
  ...
  return slides.map((slide, index) => {
    ...
    return {
      ...
      badge: {
        text:
          subtitle || (locale === "th" ? "168 Innovative" : "168 Innovative"),
...
export async function getWordPressWhy(locale: Locale): Promise<WhyItemView[]> {
  ...
  return [...items]
    .sort(
      (left, right) =>
        (left.acf?.order ?? left.id) - (right.acf?.order ?? right.id),
    )
    .map((item) => {
      const title = pickLocalizedText(
        locale,
