...
function availabilityLabel(status: string | undefined, locale: "th" | "en") {
  ...
  if (status === "in-stock" || status === "available") return { label: t.available, color: "#059669" }
  if (status === "made-to-order" || status === "mto") return { label: t.mto, color: "#d97706" }
...
export default function CatalogProductCard({ categorySlug, locale, product, view = "grid" }: Props) {
  const t = copy[locale]
  ...
  if (view === "list") {
    return (
      <article className="group flex items-start gap-4 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:border-slate-300 hover:shadow-md">
        {/* Image */}
        <Link href={href} prefetch={false} onClick={handleLinkClick} className="shrink-0">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {selectedDescription && (
            <p className="line-clamp-2 text-xs leading-5 text-slate-500">{selectedDescription}</p>
          )}
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl">
      <Link href={href} prefetch={false} onClick={handleLinkClick} className="block">
        <div className="relative aspect-4/3 overflow-hidden bg-slate-100">
          ...
          {avail && (
            <span
              ...
              style={{ background: `${avail.color}cc`, color: "#fff" }}
            >
              {avail.label}
            </span>
          )}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {group.values.map((value) => {
                    const isActive = selectedVariant?.options.some(
                      (o) => o.groupKey === group.key && o.valueLabel === value.valueLabel
                    )
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => { if (nextVariant?.slug) setSelectedVariantSlug(nextVariant.slug) }}
