...
function NavInner({ locale, logo, pathname }: NavigationProps & { pathname: string }) {
  ...
  return (
    <header
      className="sticky top-0 z-[60] w-full transition-shadow duration-300"
      style={{
        background: HOME.surface,
        borderBottom: `1px solid ${HOME.line}`,
        boxShadow: scrolled ? '0 2px 10px rgba(20,22,28,0.05)' : 'none',
      }}
    >
      <nav className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-5 lg:h-20">
        <Link href={withLocale('/')} onClick={closeMenu} className="relative flex h-10 w-32 items-center sm:h-11 sm:w-36 lg:h-12 lg:w-40">
          <Image src={logo.src} alt={logo.alt || 'Logo'} fill priority sizes="160px" className="object-contain object-left" />
        </Link>
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  className="relative inline-flex items-center text-[13px] font-bold uppercase tracking-[0.08em] transition-colors hover:opacity-70"
                  style={{ color: active ? HOME.ink : HOME.inkMid }}
                >
                  {item.label[lang]}
                  {active && (
                    <span className="absolute -bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full"
            ...
            return (
              <li key={item.href}>
                <Link
                  href={withLocale(item.href)}
                  onClick={closeMenu}
                  className="flex items-center justify-between rounded-md px-4 py-3 text-[13px] font-bold uppercase tracking-[0.06em] transition-colors"
                  style={{
