...
export default function WhoWeAre({ data }: {
  data: { title: string; content: string; image: { src: string; alt: string } }
}) {
  return (
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-4 lg:grid-cols-12">
        <div className="lg:col-span-6">
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl" style={{ ...GLASS.primary, background: SOFT_IMAGE_BG }}>
            <Image src={data.image.src} alt={data.image.alt} fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-cover" />
          </div>
        </div>
        <div className="lg:col-span-6">
