...
export default function PromoGrid({ whys, locale }: PromoGridProps) {
    ...
    <section className={`relative ${SECTION_PAD}`} style={{ background: HOME.surface }}>
      <div className={CONTAINER}>
        <div className="grid gap-4 lg:grid-cols-[1.2fr_2fr_1.2fr]">
          ...
          {/* ── Left: consult CTA (pink) ───────────────────────────── */}
          <div
            className="flex flex-col justify-between gap-8 rounded-lg p-8 sm:p-10"
            style={{ background: HOME.pink }}
          >
            <div>
              <h2
                className={`${SECTION_HEADING} text-[clamp(1.3rem,1rem+1vw,1.7rem)] font-bold`}
              ...
              >
                ...
                >
                  {item.image?.src ? (
                    ...
                    <span className="text-[15px] font-bold" style={{ color: HOME.mintInk }}>
                      {i + 1}
                    </span>
                  )}
                </div>
          ...
          {/* ── Right: story CTA (yellow) ──────────────────────────── */}
          <div
            className="flex flex-col justify-between gap-8 rounded-lg p-8 sm:p-10"
            style={{ background: HOME.yellow }}
          >
            <div>
              <p
              ...
              <h2
                className={`mt-2 ${SECTION_HEADING} text-[clamp(1.3rem,1rem+1vw,1.7rem)] font-bold`}
                style={{ color: HOME.ink }}
              >
