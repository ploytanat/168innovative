...
function formatOptionValue(key: string, value: string) {
  if (key === 'diameter_mm') return `${value} mm`
  return value.replace(/\b\w/g, (c) => c.toUpperCase())
}

/** Find the best matching product when one option value changes.
 *  Scores candidates by how many *other* option values still match the current selection. */
function findBestMatch(
  ...
  const candidates = products.filter((p) => p.specs[changedKey] === changedValue)
  if (candidates.length === 0) return current

  const scored = candidates.map((candidate) => {
    const score = optionKeys.filter(
      (k) => k !== changedKey && candidate.specs[k] === current.specs[k]
    ).length
...
export default function ProductFamilyCard({ family }: Props) {
  ...
  const selected =
    family.products.find((p) => p.sku === selectedSku) ?? family.products[0]!

  function handleSelect(key: string, value: string, e: React.MouseEvent) {
    e.preventDefault()
    const next = findBestMatch(family.products, family.optionKeys, key, value, selected)
    setSelectedSku(next.sku)
  }

  // Only show option groups that have 2+ distinct values in this (possibly filtered) family
  const visibleOptionKeys = family.optionKeys.filter((key) => {
    const unique = new Set(family.products.map((p) => p.specs[key]).filter(Boolean))
  ...
  return (
    <div className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl">
      {/* Image — clicking navigates to selected variant */}
      ...
      <div className="space-y-3 p-5">
        ...
        <Link href={`/products/${selected.sku}`}>
          <h2 className="text-base font-semibold text-slate-950 leading-snug hover:text-slate-700">
            {family.label}
          </h2>
        </Link>

        {/* Variant selectors — only shown when 2+ options exist */}
        {visibleOptionKeys.length > 0 && (
          <div className="space-y-2.5 pt-1">
            {visibleOptionKeys.map((key) => {
              // Deduplicate values, preserve first-seen product for each value
              const valueMap = new Map<string, CatalogProduct>()
              for (const p of family.products) {
                const v = p.specs[key]
                if (v && !valueMap.has(v)) valueMap.set(v, p)
              }
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from(valueMap.entries()).map(([value, product]) => {
                      ...
                      return (
                        <button
                          key={value}
                          onClick={(e) => handleSelect(key, value, e)}
